...
export default function Tarefas() {
    ...
    return (
        <div className="bebas-neue-regular min-h-screen flex justify-center bg-[#9ACFCB]">
            <div className="div-task div-container container mx-auto bg-[#D2EDEB]">
                <div className='add-tarefa-div'>
                    <p className='bebas-neue-regular add-button shadow-md'>+</p>
                    <div className="add-tarefas add-tarefas-drop shadow-md">
                        <h2>Adicionar Tarefa</h2>
                        <div className="flex flex-col items-center flex-wrap gap-3">
                            <div className='text-center'>
                                <p className='text-[#175651] tracking-wider mt-4 '>NOME DA TAREFA:</p>
                                <input 
                                    type="text"
                                    value={novaTarefa.nome}
                                    onChange={(e) => setNovaTarefa({ ...novaTarefa, nome: e.target.value })}
                                    placeholder="ESCREVA O NOME DA TAREFA"
                                    className="placeholder-[#A0F7EF] tracking-wider border-none rounded-[30px] focus:outline-none focus:ring-2 focus:ring-blue-500 text-[#02b4a4]"
                                    required
                                />
                            </div>
                            <div className='text-center'>
                                <p className='text-[#175651] tracking-wider mt-1'>RECOMPENSA DA TAREFA:</p>
                                <input 
                                    type="text"
                            ...
                            <div className='text-center'>
                                <p className='text-[#175651] tracking-wider mt-1'>SELECIONE A DIFICULDADE:</p>
                                <select
                                    value={novaTarefa.dificuldade}
                                    onChange={(e) => setNovaTarefa({ ...novaTarefa, dificuldade: e.target.value as 'Fácil' | 'Média' | 'Difícil' })}
                                    className=" text-[#02b4a4] tracking-wider border-none rounded-[30px] focus:outline-none focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <button
                                ...
                                className="tracking-wider rounded-[30px] bg-[#02b4a4] text-[#175651] hover:bg-[#175651] hover:text-[#02b4a4] transition"
                            >
                                Adicionar Tarefa
                            </button>
                        </div>
                ...
                <h1 className=" text-[#175651] mt-10">TAREFAS</h1>

                <div className="mt-[24px]">
                    {listaTarefas.map(tarefa => (
                        <div key={tarefa.id} className={`tarefas flex justify-between items-center ${tarefa.concluida ? 'bg-[#D4F8F5]' : 'bg-[#B8E8E4]'} rounded-[30px] shadow-md`}>
                            <div className="w-[80%] flex justify-between items-center">
                                ...
                                <div className='tarefas-dados flex flex-col'>
                                    <label className='leading-none tracking-widest text-[#175651]'>Nome da Tarefa:</label>
                                    <span className={`leading-none pt-3 tracking-wider ${tarefa.concluida ? 'line-through text-[#b2b2b2]' : 'text-[#02b4a4]'}`}>{tarefa.nome}</span>
                                </div>
                                <div className='tarefas-dados flex flex-col'>
                                    <label className='leading-none tracking-widest text-[#175651]'>Recompensa da Tarefa:</label>
                                    <span className={`leading-none pt-3 tracking-wider ${tarefa.concluida ? 'line-through text-[#b2b2b2]' : 'text-[#02b4a4]'}`}>{tarefa.recompensa}</span>
                                </div>
                                <div className='tarefas-dados flex flex-col'>
                                    <label className='leading-none tracking-widest text-[#175651]'>Nível de Dificuldade:</label>
                                    <span className={`leading-none pt-3 tracking-wider ${tarefa.concluida ? 'line-through text-[#b2b2b2]' : 'text-[#02b4a4]'}`}>{tarefa.dificuldade}</span>
                                </div>
                            </div>
                            <button
                                onClick={() => removerTarefa(tarefa.id)}
                                className="tracking-wider rounded-[30px] bg-[#b40202] text-[#561717] hover:bg-[#561717] hover:text-[#b40202] transition"
                            >
                                EXCLUIR
                            </button>
                        </div>
